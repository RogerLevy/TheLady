...
defer rendertile
: draw-plane ( pen=xy; col row tileparams -- ) ( pen=xy; tile -- )
   =>
   tilew 2@ locals| th tw |
   x1 2@ 2+
   ...
   rows @ bounds do
      dup i tilearray @ [][] a!
      pen 2@ 
         cols @ 0 do   @+ rendertile   tw 0 +at   loop
      th + at
   loop   drop
   2r> at   r> a!   ]batch ;
...
: draw-screenplane  ( pen=xy; scrollx scrolly planeparams -- pen=?? )
   res 2@ third fitplane   /scroll draw-plane ;

: draw-scrollplane  ( pen=xy; scrollx scrolly planeparams -- pen=?? )
   /scroll draw-plane ;
