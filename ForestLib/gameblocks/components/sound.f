...
pstruct %sound
   ...
   2 0 action (seek)   \ ( handle n. -- ) secs fixed
   1 1 action (pos)    \ ( handle -- n. ) secs fixed
...
: smprate@
   \ sndsmp @ al_get_sample_frequency dup smprate ! ;
   handle @ al_get_sample_instance_frequency dup smprate ! ;

: samps>psec s>f smprate@ s>f f/ f>p ;

: psec>samps p>f smprate@ s>f f* f>s ;
...
: seek ( n sound ) snd! handle @ swap (seek) ;
...
: playsample ( sample playmode -- )
  ...
  smpinst> handle !
  >alsmp @ dup sndsmp !
  handle @ swap al_set_sample drop
...
: sample  ( -- <name> <path> ) ( playmode -- )
   ?create namespec 2dup type sample,  does> swap playsample ;
...
: pause-sounds   snd [[ snd! handle @ if sndpause then ]] sounds each snd! ;
: resume-sounds   snd [[ snd! handle @ if sndresume then ]] sounds each snd! ;
