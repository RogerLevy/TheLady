...
4096 ?constant /segment
128 megs ?constant /maxmem
...
constant /segdescr \ segment descriptor
i]
...
: use  ( bytes subregion -- addr ) => used @ addr @ +   swap dup memused +!  used +! ;

: remaining   ( subregion -- bytes ) => size @ used @ - ;

: sort-subregions
   freed-subregions vacate
   [[ => size @ used @ 0= and if s@ freed-subregions push then ]] segments each
   [[ >size @ swap >size @ > ]] freed-subregions sort ;

: (free-subregion) ( subregion -- )
   =>   used @ negate memused +!   0 used !  0 regprev ! ;

: frontier ( -- unallkocated-segment )
...
: init-subregion  ( addr used-bytes subregion -- )
   => swap ( .s ) addr !   dup   seground dup size ! s@ >cap -1 over >used !  >size !   dup memused +! used !   ;

: new-subregion ( requested-bytes -- subregion )
   here over seground allot frontier =>
...
: use-any-unused-subregion ( requested-bytes -- addr subregion | 0 )
   ...
   freed-subregions itemsbounds ?do
      i @ >size @ b 4 * >= if false unloop exit then   \ early out - i'd rather create new subregions than waste much larger ones (and to avoid combine/split for now 6/24)
...
: trim ( region bytes -- )
   dup 0< abort" Negative trim amount"
   locals| b | =>
   total @ 0= ?exit
   total @ b = ?exit
   b s@ reserved > abort" Trim amount more than region size"
   s@ dup reserved b - 
...
: reserve ( region bytes -- addr )
   ...
   b use-any-unused-subregion ?dup 0= if
      report" new sr"
      b new-reservation
   else
