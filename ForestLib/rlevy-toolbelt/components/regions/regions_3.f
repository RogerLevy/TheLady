...
: sort-subregions
  freed-subregions _vacate
  [[ with size @ used @ 0= and if s@ freed-subregions _push then ]] segments _each
  [[ .size @ swap .size @ > ]] freed-subregions _sort ;

: (free-subregion) ( subregion -- )
  with  used @ negate memused +!  0 used !  0 regprev ! ;

: frontier ( -- unallocated-segment )
  allocated @ /segment / segments _[] ;
...
: (print-subregions)  ( subregion n | 0 -- )
   1 +
   over if
      2dup cr ." SUBREGION " . ." :" .
      >r dup print-subregion
      .regprev @ r> recurse
   else
      2drop
   then
...
\ bytes is the number of bytes left after the trim.
: TRIM ( region bytes -- )
  0 max locals| b | with
  total @ 0= ?exit
  total @ b = ?exit
