...
\ window stuff
void function: al_set_new_window_position ( int-x int-y -- )
void function: al_set_window_position ( ALLEGRO_DISPLAY-*display int-x int-y -- )
void function: al_set_new_display_adapter ( int-adapter -- )

function: al_install_system ( ver atexit -- bool ) \ (int version int (*atexit_ptr)(void (*)(void))));
void function: al_uninstall_system ( -- )
: al_init     ALLEGRO_VERSION_INT 0 al_install_system ;
void function: al_inhibit_screensaver ( bool -- )
...
void function: al_get_mouse_state     ( ALLEGRO_MOUSE_STATE-*ret_state -- )
...
void function: al_show_mouse_cursor ( struct-ALLEGRO_DISPLAY-*display -- )
void function: al_hide_mouse_cursor ( struct-ALLEGRO_DISPLAY-*display -- )
void function: al_grab_mouse ( struct-ALLEGRO_DISPLAY-*display -- )
void function: al_ungrab_mouse ( -- )
...
void function:    al_get_keyboard_state ( ALLEGRO_KEYBOARD_STATE-*ret_state -- )
...
void function:           al_release_joystick    ( ALLEGRO_JOYSTICK-* -- )
void function:           al_get_joystick_state  ( ALLEGRO_JOYSTICK-* ALLEGRO_JOYSTICK_STATE-*ret_state -- )

\ display
/* Possible bit combinations for the flags parameter of al_create_display. */
#define   ALLEGRO_WINDOWED                     1 0 <<
#define   ALLEGRO_FULLSCREEN                   1 1 <<
#define   ALLEGRO_OPENGL                       1 2 <<
#define   ALLEGRO_DIRECT3D_INTERNAL            1 3 <<
#define   ALLEGRO_RESIZABLE                    1 4 <<
#define   ALLEGRO_NOFRAME                      1 5 <<
#define   ALLEGRO_GENERATE_EXPOSE_EVENTS       1 6 <<
#define   ALLEGRO_OPENGL_3_0                   1 7 <<
#define   ALLEGRO_OPENGL_FORWARD_COMPATIBLE    1 8 <<
#define   ALLEGRO_FULLSCREEN_WINDOW            1 9 <<
#define   ALLEGRO_MINIMIZED                    1 10 <<
...
void function: al_destroy_display ( ALLEGRO_DISPLAY-*display -- )
function: al_get_backbuffer   ( ALLEGRO_DISPLAY-*display -- bitmap )

void function: al_set_target_bitmap ( ALLEGRO_BITMAP-*bitmap -- )
void function: al_set_target_backbuffer ( ALLEGRO_DISPLAY-*display -- )
function: al_resize_display     ( ALLEGRO_DISPLAY-*display int-width int-height -- bool )
void function: al_flip_display       ( -- )
void function: al_update_display_region ( int-x int-y int-width int-height -- )
...
#define   ALLEGRO_TEXTLOG_MONOSPACE            1 1 <<
...
void function: al_destroy_native_file_dialog ( ALLEGRO_FILECHOOSER-*dialog -- )
...
void function: al_close_native_text_log ( ALLEGRO_TEXTLOG-*textlog -- )
\ void function: al_append_native_text_log ( ALLEGRO_TEXTLOG-*textlog char-const-*format ... -- )
