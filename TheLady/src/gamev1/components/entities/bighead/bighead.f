...
: billboard   >r %billboard ~> over script r> bitmap! endp ;
...
%actor ~> %bighead
   ...
   : removered   red1 @ delete red2 @ delete ;
   : in&out   0 perform lifetime @ 1p psin 0.1 p* 0.9 + alpha! ;
...
%bighead script
   ...
   \ hitbox: bighead.image 0.7 center-bitmap-box ;

   0.5 0.5 pivot 2!
   bighead-cflags cflags !
   
   :: wake  me 0 0 from %bigheadintro oneslave drop vis off nod ;

   start: 0 perform   player 750.0 close? if wake then ;
