/*
There could be an offset problem.... tweak the original https://www.shadertoy.com/view/MdlGDM#
...
void main(void)
{
	...
	vec3 v;
	vec3 p = 4.*vec3(uv,0.)+iGlobalTime*(.01,.07,.2);
	float x = myfbm(p);
	//v = vec3(x);
	v = (.5+.5*sin(x*vec3(5.,5.,5.)*SCALE))/SCALE;
	float g = 1.;
	//g = pow(length(v),1.);
